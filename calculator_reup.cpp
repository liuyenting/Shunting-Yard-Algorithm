...
class ShuntingYard
{
	...
	int _stackprecedence () const
	{
		if (_op_stack.empty ()) { return -1; }
		return precedence (_op_stack.top ());
	}
	/* Reset precedence to allow for new scope */
	void handle_left_paren () { _op_stack.push ('('); }
	/* Consume all operators in current scope and restore previous scope */
	void handle_right_paren ()
	{
		while ('(' != _op_stack.top ())
		{
			rpn_.push (new Token< char >(_op_stack.top ()));
			_op_stack.pop ();
		}
		_op_stack.pop ();
	}
	...
	void handle_op (char op)
	{
		while (! _op_stack.empty () &&
		        precedence (op) <= _stackprecedence ())
		{
			rpn_.push (new Token< char >(_op_stack.top ()));
			_op_stack.pop ();
		}
